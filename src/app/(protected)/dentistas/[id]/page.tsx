...
import { DentistaRequest, DentistaResponse } from "@/models/dentista.type";
...
export default function EditarDentistaPage() {
    ...
    const carregarDentista = async () => {
        try {
            ...
            const data = await DentistaService.buscarPorId(id);
            setDentista(data);
            setFormData({
                nome: data.nome,
                cro: data.cro,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            const id = Number(params.id);
            await DentistaService.atualizar(id, formData);
            toast.success("Dentista atualizado com sucesso");
            router.push("/dentistas");
        } catch (error) {
            toast.error("Erro ao atualizar dentistas");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="container mx-auto py-10 max-w-2xl">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Atualize os dados do dentista {dentista?.nome}
                    </CardDescription>
                </CardHeader>
                <CardContent>
